...
namespace GE
{
	...
	void GSceneSerializer::Serialize(GScene* scene, YAML::Emitter& serializingData)
	{
		auto& SerializeData = serializingData;
	...
	void GSceneSerializer::Export(const std::string& filePath, GScene* scene)
	{
		YAML::Emitter SerializeData;
		Serialize(scene, SerializeData);
	...
	void GSceneSerializer::Import(const std::string& filePath, GScene* scene)
	{
		Deserialize(scene, Load(filePath));
	}
