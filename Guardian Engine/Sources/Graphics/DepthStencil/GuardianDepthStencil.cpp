...
namespace GE
{
	...
	void GuardianDepthStencil::InitializeDepthStencil(std::shared_ptr<GuardianGraphics> graphics, int width, int height, bool isDepthGraph)
	{
		...
		DepthStencilBufferDesc.Format = isDepthGraph == true ? DXGI_FORMAT_R32_TYPELESS : DXGI_FORMAT_R24G8_TYPELESS;
		...
		DepthStencilViewDesc.Format = isDepthGraph == true ? DXGI_FORMAT_D32_FLOAT : DXGI_FORMAT_D24_UNORM_S8_UINT;
		...
		DepthStencilShaderResourceDesc.Format = isDepthGraph == true ? DXGI_FORMAT_R32_FLOAT : DXGI_FORMAT_R24_UNORM_X8_TYPELESS;
	...
	void GuardianDepthStencil::ClearDepthStencil(std::shared_ptr<GuardianGraphics> graphics)
	{
		...
		graphics->GetGraphicsDeviceContext()->OMSetDepthStencilState(this->DepthStencilState.Get(), 0);
	}
