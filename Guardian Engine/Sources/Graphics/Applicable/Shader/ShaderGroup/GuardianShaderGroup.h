...
namespace GE
{
	...
	public:
		GuardianShaderGroup();
		GuardianShaderGroup(const GString& name, std::shared_ptr<GuardianVertexShader> vs,
		...
		void InitializeShaderGroup(const GString& name, std::shared_ptr<GuardianVertexShader> vs,
			D3D11_INPUT_ELEMENT_DESC layoutData[], int dataSize, std::shared_ptr<GuardianPixelShader> ps);
